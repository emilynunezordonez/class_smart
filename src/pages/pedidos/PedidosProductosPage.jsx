...
export function PedidosProductosPage() {
  ...
  useEffect(() => {
    async function fetchPedidosProductos() {
      try {
        ...
        const filteredPedidosProductos = response.data.filter(
          (pedidoProducto) => pedidoProducto.pedido_ppid == id
        );
  ...
  const handleCancelButtonClick = async () => {
    ...
    try {
      await send_cancel_mail(
        usuario.email,
        `Su pedido N° ${id} ha sido cancelado`
      );
    ...
    <div>
      <div className="grid grid-cols-3 gap-3 mt-8">
        ...
        {pedidosProductos.map((producto, index) => (
          <PedidoProductoCard
            key={index}
            productId={producto.producto_ppid}
            cantidad={producto.cantidad_producto_carrito}
          />
        ))}
      </div>
      <p
        className="text-slate-900 text-2xl mt-1 ml-5"
        data-testid="total-precio"
      >
        <strong>TOTAL PEDIDO: </strong> ${totalPrice.toLocaleString("es-ES")}
      </p>
